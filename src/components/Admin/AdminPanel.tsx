...
export function AdminPanel({ matches, onRefresh, onClose }: any) {
  ...
  const handleUpdate = async (id: number) => {
    ...
    // Capturamos los penales
    const penHomeVal = (document.getElementById(`pen-home-${id}`) as HTMLInputElement).value;
    const penAwayVal = (document.getElementById(`pen-away-${id}`) as HTMLInputElement).value;
    
    const penalty_home = penHomeVal !== "" ? parseInt(penHomeVal) : null;
    const penalty_away = penAwayVal !== "" ? parseInt(penAwayVal) : null;
      ...
      <div className="max-w-md mx-auto space-y-6">
        <div className="flex justify-between items-center border-b border-zinc-800 pb-4 sticky top-0 bg-black/50 py-2">
          <div>
            ...
            <p className="text-xs text-zinc-400">Control de resultados y penales</p>
          </div>
            ...
            <div key={m.id} className="bg-zinc-900/50 border border-zinc-800 p-4 rounded-3xl space-y-4">
              ...
              <div className="grid grid-cols-3 items-center gap-2">
                ...
                <div className="flex flex-col gap-2 justify-center items-center">
                   ...
                   {/* Entradas para Penales (NUEVO) */}
                   <div className="flex gap-2 items-center bg-amber-950/20 p-1 rounded-lg border border-amber-900/30">
                     <span className="text-[8px] text-amber-500 uppercase font-bold px-1">Pen:</span>
                     <input id={`pen-home-${m.id}`} type="number" defaultValue={m.penalty_home ?? ''} placeholder="-" className="w-7 h-6 bg-zinc-950 border border-amber-900/50 rounded text-center text-xs font-bold text-amber-500"/>
                     <input id={`pen-away-${m.id}`} type="number" defaultValue={m.penalty_away ?? ''} placeholder="-" className="w-7 h-6 bg-zinc-950 border border-amber-900/50 rounded text-center text-xs font-bold text-amber-500"/>
                   </div>
                </div>
